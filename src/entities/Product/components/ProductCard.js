...
export const ProductCard = ({ product, isLoading }) => {
    ...
    const sanitizedDescriptionHtml = sanitizeHtml(removeImageLinks(descriptionHtml));

    const { getPrefixCls } = useContext(ConfigContext);
    const uiLibPrefix = getPrefixCls('card');
    const uiLibMetaCN = classNames(`${uiLibPrefix}-meta`);
    const descriptionCN = classNames(`${uiLibPrefix}-meta-description`, 'product-html-description');
    return (
        <UiCard
            bordered={false}
            loading={isLoading}
            ...
            extra={<CanvasImg alt={img?.altText ?? 'product photo'} style={productCardStyle.img} src={img?.url ?? ''} />}
        >
            <div className={uiLibMetaCN} style={productCardStyle.meta}>
                <div className={`${uiLibPrefix}-meta-detail`}>
                    {title ? <div className={`${uiLibPrefix}-meta-title`}>{title}</div> : null}
                    <div className={descriptionCN} style={productCardStyle.metaDescr}>{descriptionHtml ? <div dangerouslySetInnerHTML={{ __html: sanitizedDescriptionHtml }} /> : null}</div>
                </div>
            </div>
        </UiCard>
    );
